...
router.get('/', requireAuth, (req, res) => {
  ...
  const actions = db.listScheduledActionsByUser(userId);
...
router.post('/', requireAuth, (req, res) => {
  ...
  const action = db.createScheduledAction({
    userId,
...
router.delete('/:id', requireAuth, (req, res) => {
  ...
  const action = db.getScheduledAction(id);
  ...
  scheduler.cancel(id);
  db.updateScheduledAction(id, { status: 'canceled' });
  res.json({ ok: true });
});
